...
test("Typography - Body text larger than default must use .text-body-lg class", () => {
  const errors: string[] = [];

  htmlFiles.forEach(file => {
    const content = readFileSync(file, "utf-8");

    // Check for paragraphs with text-lg or other size classes (should use .text-body-lg instead)
    const prohibitedSizeClasses = [
      /<p[^>]*class="[^"]*text-lg/,
      /<p[^>]*class="[^"]*text-xl/,
      /<p[^>]*class="[^"]*text-2xl/,
    ];

    prohibitedSizeClasses.forEach(pattern => {
      const matches = content.match(new RegExp(pattern.source, 'g'));
      if (matches) {
        // Filter out comments
        const realMatches = matches.filter(m => !m.includes('<!--') && !m.includes('//'));
        if (realMatches.length > 0) {
          errors.push(`${file}: Found paragraph with size class (${realMatches.length} instances) - use .text-body-lg for 18px body text instead of Tailwind size classes`);
        }
      }
    });
  });
  ...
  if (errors.length > 0) {
    throw new Error(`Body text size violations found:\n${errors.join('\n')}`);
  }
});
...
test("Preline Components - Collapse must have required data attribute", () => {
  const errors: string[] = [];

  htmlFiles.forEach(file => {
    const content = readFileSync(file, "utf-8");

    // Check for hs-collapse-toggle buttons
    const hasCollapseToggle = /hs-collapse-toggle/.test(content);
    if (hasCollapseToggle) {
      // Must have data-hs-collapse attribute
      if (!/data-hs-collapse=/.test(content)) {
        errors.push(`${file}: hs-collapse-toggle found but missing data-hs-collapse attribute`);
      }
      // Must have corresponding hs-collapse element
      if (!/class="[^"]*hs-collapse/.test(content)) {
        errors.push(`${file}: hs-collapse-toggle found but missing hs-collapse element`);
      }
    }
  });

  if (errors.length > 0) {
    throw new Error(`Preline collapse violations found:\n${errors.join('\n')}`);
  }
});

test("Preline Components - Dropdown must have required structure", () => {
  const errors: string[] = [];

  htmlFiles.forEach(file => {
    const content = readFileSync(file, "utf-8");

    // Check for hs-dropdown containers
    const hasDropdown = /hs-dropdown/.test(content);
    if (hasDropdown) {
      // Must have hs-dropdown-toggle
      if (!/hs-dropdown-toggle/.test(content)) {
        errors.push(`${file}: hs-dropdown found but missing hs-dropdown-toggle`);
      }
      // Must have hs-dropdown-menu
      if (!/hs-dropdown-menu/.test(content)) {
        errors.push(`${file}: hs-dropdown found but missing hs-dropdown-menu`);
      }
    }
  });

  if (errors.length > 0) {
    throw new Error(`Preline dropdown violations found:\n${errors.join('\n')}`);
  }
});

test("Tag Pattern - Must match documented pattern", () => {
  const errors: string[] = [];

  htmlFiles.forEach(file => {
    const content = readFileSync(file, "utf-8");

    // Find all tag-like elements (span with py-1.5 px-3)
    const tagPattern = /<span[^>]*class="[^"]*py-1\.5[^"]*px-3[^"]*">/g;
    const tagMatches = content.match(tagPattern);

    if (tagMatches) {
      tagMatches.forEach(match => {
        // Check for required classes in tag pattern
        const hasRequired = 
          /py-1\.5/.test(match) &&
          /px-3/.test(match) &&
          /bg-white/.test(match) &&
          /text-gray-800/.test(match) &&
          /border/.test(match) &&
          /border-gray-200/.test(match) &&
          /text-sm/.test(match) &&
          /rounded-xl/.test(match);

        // Check for prohibited classes
        const hasProhibited = 
          /text-gray-600/.test(match) || // Should be text-gray-800
          /text-xs sm:text-sm/.test(match) || // Should be text-sm only
          /rounded-full/.test(match); // Should be rounded-xl

        if (!hasRequired || hasProhibited) {
          errors.push(`${file}: Tag pattern doesn't match documented pattern. Found: ${match.substring(0, 100)}`);
        }
      });
    }
  });
  ...
  if (errors.length > 0) {
    throw new Error(`Tag pattern violations found:\n${errors.join('\n')}`);
  }
});

test("Social Icon Pattern - Must match documented pattern", () => {
  const errors: string[] = [];

  htmlFiles.forEach(file => {
    const content = readFileSync(file, "utf-8");

    // Find social icon links (size-8 with rounded-full and icon)
    const socialPattern = /<a[^>]*class="[^"]*size-8[^"]*rounded-full[^"]*"[^>]*>[\s\S]*?<i class="ph ph-(linkedin|x|github|twitter)[^"]*">/g;
    const socialMatches = content.match(socialPattern);
    ...
    if (socialMatches) {
      socialMatches.forEach(match => {
        ...
        const hasRequired = 
          /size-8/.test(match) &&
          /rounded-full/.test(match) &&
          /text-gray-500/.test(match) &&
          /hover:bg-gray-50/.test(match);

        if (!hasRequired) {
          errors.push(`${file}: Social icon pattern doesn't match documented pattern`);
        }
      });
    }
  });
  ...
  if (errors.length > 0) {
    throw new Error(`Social icon pattern violations found:\n${errors.join('\n')}`);
  }
});
